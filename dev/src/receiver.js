...
(function (global) {
  ...
  function step() {
    ...
    if (now - lastRelocate > 30000) needRelocate = true;  /* 周期性重新定位防漂移 */
    drawOverlay();
    updateUI();
  }
  ...
  function doRelocate() {
    ...
    /* 阶段 B：由种子码推导 3×2 网格，候选位置紧裁剪逐个解码 */
    ...
    for (var i = 0; i < offsets.length; i++) {
      ...
      if (qr && qr.binaryData) {
        ...
        if (pkt) {
          found.push({ x: rx, y: ry, w: side, h: side, fails: 0, ok: 1 });
          handlePacket(pkt);
        }
      }
    }
    if (found.length) {
      ...
      setStatus('已锁定 ' + regions.length + ' 个二维码，接收中…');
  ...
  function handlePacket(pkt) {
    if (pkt.isMeta) {
      try {
        var m = JSON.parse(new TextDecoder().decode(pkt.payload));
    ...
    if (!meta || pkt.index < 1 || pkt.index > meta.chunks) return;
